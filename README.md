...
The Pluto Enterprise application image is a private package in GitHub Container Registry. Each enterprise receives a read-only package identity from Pluto. The enterprise never needs access to the private `sejati-app` repository.
...
- `ghcr.io/juxtalabs/pluto-enterprise-api` remains private. It holds two compiled binaries (the server, which also serves the dashboard, and its admin CLI) and no application source.
